...
/**
 * A pipeline job that manages the lifecycle of a bigquery load {@link Job}. It triggers,
 * polls for status and retries or cleans up the files based on the status of the load
 * job.
 */
...
final class BigQueryLoadFileSetJob extends Job1<BigQueryLoadJobReference, Integer> {
	...
	private BigQueryLoadJobReference triggerBigQueryLoadJob() {
		...
		try {
			Insert insert = BigQueryLoadGoogleCloudStorageFilesJob.getBigquery().jobs().insert(projectId, job);
			Job executedJob = insert.execute();
			log.info("Triggered the bigQuery load job for files " + fileSet + " . Job Id = " + executedJob.getId());
			return new BigQueryLoadJobReference(projectId, executedJob.getJobReference());
		} catch (IOException e) {
			log.warning("Error in triggering BigQuery load job for files " + fileSet);
			throw new RuntimeException("Error in triggering BigQuery load job for files " + fileSet, e);
		}
	}

	/**
	 * Create a {@link Job} instance for the specified files and bigquery
	 * {@link TableSchema} with default settings.
	 */
...
//				+ "\nSchema: " + loadConfig.getSchema());
	...
	public Value<BigQueryLoadJobReference> run(Integer retryCount) throws Exception {
		...
		return futureCall(new RetryLoadOrCleanupJob(dataset, tableName, projectId, fileSet, schema), jobTrigger, immediate(retryCount),
