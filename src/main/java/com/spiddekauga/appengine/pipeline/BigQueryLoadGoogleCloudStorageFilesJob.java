...
public final class BigQueryLoadGoogleCloudStorageFilesJob extends
	...
	private static final Logger log = Logger.getLogger(BigQueryLoadGoogleCloudStorageFilesJob.class.getName());
	...
	static Bigquery getBigquery() {
		...
		return new Bigquery.Builder(HTTP_TRANSPORT, JSON_FACTORY, credential).setHttpRequestInitializer(credential)
				.setGoogleClientRequestInitializer(initializer).setApplicationName(SystemProperty.applicationId.get()).build();
	}
	 ...
	 */
	public BigQueryLoadGoogleCloudStorageFilesJob(String dataset, String tableName, String projectId) {
	...
	/**
	 * Divides the files into bundles having size less than or equal to the maximum size
	 * allowed per bigquery load {@link Job} and then starts separate pipeline jobs for
	 * each of the bundles. Returns a list of {@link JobReference}s.
	 */
	@Override
	public Value<List<BigQueryLoadJobReference>> run(BigQueryStoreResult<GoogleCloudStorageFileSet> bigQueryStoreResult) throws Exception {
		...
		List<List<GcsFilename>> bundles = bundleFiles(files, BigQueryConstants.BIGQUERY_LOAD_DATA_SIZE_LIMIT);
		List<FutureValue<BigQueryLoadJobReference>> jobReferenceList = new ArrayList<>();
		for (List<GcsFilename> bundle : bundles) {
			jobReferenceList.add(futureCall(
					new BigQueryLoadFileSetJob(dataset, tableName, projectId, bundle, SerializableValue.of(
							Marshallers.getGenericJsonMarshaller(TableSchema.class), outputResult.getSchema())), immediate(Integer.valueOf(0)),
					onQueue(Optional.fromNullable(getOnQueue()).or("default"))));
		}
	 ...
	 * @return List of files bundled together so that the combined size of the files is
	 *         less than the specified bundle size limit.
	 * @throws IOException
	 */
	private List<List<GcsFilename>> bundleFiles(List<GcsFilename> files, long bundleSizeLimit) throws IOException {
	...
	public Value<BigQueryStoreResult<GoogleCloudStorageFileSet>> handleException(Throwable t) throws Throwable {
	...
	public String getJobDisplayName() {
		return "BigQueryLoadGoogleCloudStorageFile";
	}
}
