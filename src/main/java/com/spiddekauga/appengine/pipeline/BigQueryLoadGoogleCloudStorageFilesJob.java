 ...
 * bigquery load {@link Job} and then starts separate pipeline jobs for each of the bundles. Returns
 * a list of {@link JobReference}s.
 */
@Override
public Value<List<BigQueryLoadJobReference>> run(BigQueryStoreResult<GoogleCloudStorageFileSet> bigQueryStoreResult) throws Exception {
	...
	for (List<GcsFilename> bundle : bundles) {
		jobReferenceList.add(futureCall(
				new BigQueryLoadFileSetJob(mDataset, mTableName, mProjectId, bundle, SerializableValue.of(
