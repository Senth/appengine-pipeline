...
/**
 * A pipeline job to poll the status of a bigquery load {@link Job}. It polls until the
 * job goes to completion or it fails.
 */
	...
	public Value<Void> run(final BigQueryLoadJobReference jobToPoll) throws Exception {
		DeferredTask pollTask = new DeferredTask() {
			...
			public void run() {
				...
				try {
					Job pollJob = BigQueryLoadGoogleCloudStorageFilesJob.getBigquery().jobs().get(jobToPoll.getJobReference().getProjectId(), jobRef)
							.execute();
					log.info("Job status of job " + jobRef + " : " + pollJob.getStatus().getState());
					if (pollJob.getStatus().getState().equals("PENDING") || pollJob.getStatus().getState().equals("RUNNING")) {
						setForRetry();
					} else {
						submitPromisedValue(pollJob.getStatus().getState());
					}
				} catch (IOException e) {
					log.warning("Unable to poll the status of the job " + jobRef + " . Retrying after " + BigQueryConstants.MIN_TIME_BEFORE_NEXT_POLL
							+ " seconds");
					setForRetry();
				}
			}
			...
			private void submitPromisedValue(String status) {
				try {
					...
					try {
						if (State.RUNNING.equals(PipelineManager.getJob(getPipelineKey().getName()).getJobState())) {
							throw new RuntimeException(
									"No handle found for the promise. Root job is still running. So task queue will retry. Pipeline url. "
											+ getStatusConsoleUrl());
						}
		...
		queue.add(TaskOptions.Builder
				.withPayload(pollTask)
				.countdownMillis(10000)
				.retryOptions(
						RetryOptions.Builder.withMinBackoffSeconds(BigQueryConstants.MIN_TIME_BEFORE_NEXT_POLL).maxBackoffSeconds(
